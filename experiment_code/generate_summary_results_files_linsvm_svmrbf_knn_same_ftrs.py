...
RESULTS_DIR = './linsvm_svmrbf_knn_same_ftrs_results/'
JSON_FNAME = './data_for_testing/data_by_bird.JSON'
SHELVE_BASE_FNAME = 'test_same_ftr_results_'
RESULTS_SHV_BASE_FNAME = '_test_same_ftr_summary_results'
...
for bird_ID, bird_data in data_by_bird.items():
    ...
    svm_train_fname = os.path.join(DATA_DIR + bird_data['svm_train_feat']) # just used svm feature files! not knn features
    svm_test_fname = os.path.join(DATA_DIR + bird_data['svm_test_feat']) # even though I tested a knn model
    ...
    svm_Tach_test_cm_arr = np.empty((rows,cols),dtype='O')
    ...
    for col_ind, num_songs in enumerate(NUM_SONGS_TO_TEST):
        for row_ind,replicate in enumerate(REPLICATES):
            print('loading shelve file for {0}, {1} songs, replicate {2}'.format(bird_ID,num_songs,replicate+1))
            shelve_fname = RESULTS_DIR + \
                            SHELVE_BASE_FNAME + \
                            str(bird_ID) + ", " + str(num_songs) + \
                            ' songs, replicate ' + str(replicate + 1) + '.db'
            ...
            with shelve.open(shelve_fname,'r') as replicate_shv:
                ...
                try:
                    # make sure 'test_labels' from replicate shelve file == original 'svm_test_labels' from .mat data file
                    assert np.array_equal(test_labels,svm_test_labels)
                ...
                acc_by_label,avg_acc = get_acc_by_label(linsvm_test_labels_no_intro,linsvm_test_no_intro_pred_labels,non_intro_note_labelset)
